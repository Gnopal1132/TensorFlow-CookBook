...
"""
Here we are going to combine everything we have learned so far. Here I will write everything.
"""
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, x, mask=None, to_print=True):
...
class EncoderLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, ffn_hidden, num_heads, drop_prob, print_=True, **kwargs):
        super(EncoderLayer, self).__init__(**kwargs)
        self.d_model = d_model
        self.ffn_hidden = ffn_hidden
        self.num_heads = num_heads
        self.to_print = print_
        self.drop_factor = drop_prob

        # Let's Define Layers
        self.attention = MultiHeadAttention(d_model=self.d_model, num_head=self.num_heads)
        self.norm1 = LayerNormalization(param_shape=[self.d_model])
        self.drop1 = tf.keras.layers.Dropout(rate=self.drop_factor)
        ...
        self.drop2 = tf.keras.layers.Dropout(rate=self.drop_factor)
...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, d_model, ffn_hidden, num_heads, drop_prob,
                 num_layers, max_sequence_length, source_vocab, **kwargs):
        ...
        self.data_layer = SentenceEmbedding(max_sequence_length=max_sequence_length,
                                            d_model=d_model, vocab_size=source_vocab,
                                            drop_rate=drop_prob)
        for _ in range(num_layers):
            self.layers.append(EncoderLayer(d_model, ffn_hidden, num_heads, drop_prob))
...
if __name__ == '__main__':
    ...
    encoder = Encoder(d_model, feed_forward_size, num_heads, dropout_prob, num_layers, max_seq_len, vocab_size)
    x = tf.random.uniform(shape=(batch_size, max_seq_len), minval=0, maxval=1000)
    out = encoder(x)
